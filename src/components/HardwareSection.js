...
function HardwareSection({ initialHardware = [], onQuantityUpdate }) { // ✅ added onQuantityUpdate prop
  const hardwareList = Array.isArray(initialHardware) ? initialHardware : [];
  const [hardware, setHardware] = useState(
    hardwareList.length > 0
      ? hardwareList
  ...
  // ✅ helper to compute CPU and GPU quantities and pass to parent
  const updateCpuGpuCounts = (updatedHardware) => {
    const cpuQty = updatedHardware
      .filter(h => h.type === 'CPU')
      .reduce((sum, h) => sum + h.quantity, 0);
    const gpuQty = updatedHardware
      .filter(h => h.type === 'GPU')
      .reduce((sum, h) => sum + h.quantity, 0);

    onQuantityUpdate(cpuQty, gpuQty); // ✅ notify parent
  };
  ...
  const addHardware = () => {
    ...
    if (selected) {
      ...
      setHardware(prev => {
        const updated = [...prev, newHardware];
        updateCpuGpuCounts(updated);
        return updated;
      });
  ...
  const updateQuantity = (id, increase) => {
    setHardware(prev => {
      ...
      updateCpuGpuCounts(updated); // ✅ notify parent
      return updated;
    });
  };
